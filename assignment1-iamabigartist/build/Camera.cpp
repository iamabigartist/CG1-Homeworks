...
void Camera::RefreshMatrixByInput()
{
	...
	//Camera move
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) { position += forward * move_speed * (float)delta_time; }
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) { position -= forward * move_speed * (float)delta_time; }
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) { position -= right * move_speed * (float)delta_time; }
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) { position += right * move_speed * (float)delta_time; }
