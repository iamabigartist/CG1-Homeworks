...
int main ( int argc, char* argv [] )
{
	...
	/////////////////////////////////////////////////////////////////////////
	string confs [] = { "config_spec_trans.txt" ,"config_all_diff.txt" };
	if ( argc >= 2 )
	{
		if ( !conf.parseConfigFile ( "../configs/" + confs [ stoi ( argv [ 1 ] ) ] ) )
		{
			return -1;
		}
		conf.printConfig ();
	...
	TriangleMesh mesh_dragon ( "../resources/stanford_dragon.obj", &mat_dragon, 3 );
	mesh_dragon.applyTransformation ( Eigen::Translation3f ( 2.5, -7.0, -6.5 ) * Eigen::Scaling ( 25.0f ) );
	mesh_dragon.buildAccelerationStructure ();

	TriangleMesh mesh_bunny ( "../resources/stanford_bunny.obj", mat_bunny, 3 );
	mesh_bunny.applyTransformation ( Eigen::Translation3f ( -2, -6.5, -6.0 ) * Eigen::Scaling ( 25.0f ) );
	mesh_bunny.buildAccelerationStructure ();
	...
	PathTracingIntegrator integrator ( scene, camera, conf );
	integrator.render ();
